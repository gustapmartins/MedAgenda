...
const GoBackHeader = ({ onPress, onPressIcon, title, icon }: TypeProps) => {
  return (
    <View style={icon ? styles.header : stylesIconBoolean.header}>
      <TouchableOpacity style={icon ? styles.backButton : stylesIconBoolean.backButton} onPress={onPress}>
        <Icon name="arrow-left" size={28} color="#333" />
      </TouchableOpacity>
      <Text style={styles.headerTitle}>{title}</Text>

      {icon && (
        <TouchableOpacity style={icon ? styles.backButton : stylesIconBoolean.backButton} onPress={onPressIcon}>
